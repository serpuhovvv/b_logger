...
@pytest.mark.xfail
def test_playwright_should_pass(playwright_page):  #  <-- Will be detected automatically
    with blog.step('Open any URL'):
...
@pytest.mark.xfail
def test_playwright_should_fail(playwright_page):  #  <-- Will be detected automatically
    with blog.step('Open any URL'):
        playwright_page.goto(f'https://google.com')
